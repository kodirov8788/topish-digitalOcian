...
const uploadFileToS3 = async (buffer, name, type) => {
    ...
    return `https://${process.env.AWS_BUCKET_NAME}.s3.${process.env.AWS_S3_BUCKET_REGION}.amazonaws.com/avatar/${name}`;
};
...
class AvatarCTRL {
    ...
    async uploadAvatar(req, res, next) {
        ...
        upload(req, res, async (err) => {
            ...
            if (!req.file) {
                ...
                return handleResponse(res, 400, 'error', 'Please provide an image to upload.', null, 0);
            }
            ...
            try {
                ...
                console.error(error);
                return handleResponse(res, 500, 'error', 'Unable to update user avatar.', null, 0);
            }
        });
    }
    ...
    async getAvatar(req, res) {
        try {
            ...
            Body.pipe(res).on('error', (streamError) => {
                ...
                return handleResponse(res, 500, 'error', 'Error streaming avatar image', null, 0);
            });

            return handleResponse(res, 200, 'success', 'Avatar retrieved successfully', null, 0);
        } catch (error) {
    ...
    async deleteAvatar(req, res) {
        try {
            ...
            return handleResponse(res, 200, 'success', 'Avatar deleted successfully', null, 0);
        } catch (error) {
