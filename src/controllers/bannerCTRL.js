...
const fileFilter = (req, file, cb) => {
    ...
    if (allowedMimeTypes.includes(file.mimetype)) {
        ...
        cb(new Error("Invalid file format. Only images (jpeg, jpg, png) are allowed."), false);
    }
};
...
class BannerCTRL {
    async createBanner(req, res) {
        ...
        upload(req, res, async (err) => {
            if (err) {
                return handleResponse(res, 400, 'error', 'Upload Error: ' + err.message, null, 0);
            }
            try {
                ...
                let uploadPromises = files.map(async file => {
                    ...
                    return `https://${process.env.AWS_BUCKET_NAME}.s3.${process.env.AWS_S3_BUCKET_REGION}.amazonaws.com/${fileName}`;
                });
                ...
                if (banners.length < 1) {
                    banner = new Banner({
                        bannerImages: imageUrls.filter(url => url !== 'Non-PNG file skipped or default image URL') // Filter out placeholders
                    });
                } else {
                    banner = banners[0];
                    banner.bannerImages.push(...imageUrls.filter(url => url !== 'Non-PNG file skipped or default image URL')); // Filter out placeholders
                }

                await banner.save();
                return handleResponse(res, 200, 'success', 'Banner updated successfully', banner, 1);
            } catch (error) {
    ...
    async deleteBanner(req, res) {
        ...
        try {
            ...
            banner.bannerImages = banner.bannerImages.filter(image => image !== imageUrl);
            await banner.save();

            return handleResponse(res, 200, 'success', 'Image deleted successfully', banner);
        } catch (error) {
            return handleResponse(res, 500, 'error', 'Something went wrong: ' + error.message, null, 0);
        }
    }
    async getAllBanner(req, res) {
        ...
        try {
            ...
            return handleResponse(res, 200, 'success', 'Banners retrieved successfully', ...banners, banners.length);
        } catch (error) {
            return handleResponse(res, 500, 'error', 'Something went wrong: ' + error.message, null, 0);
        }
    }
    async moveIndexPosition(req, res) {
        try {
            ...
            if (oldIndex < 0 || oldIndex >= banner.bannerImages.length || newIndex < 0 || newIndex >= banner.bannerImages.length) {
