...
class OfficesCTRL {
    async createOffice(req, res) {
        try {
            ...
            return handleResponse(res, 201, 'success', 'Office created successfully', Office, 1);
        } catch (error) {
    ...
    async deleteOffice(req, res, next) {
        try {
            ...
            if (!deleteJob) {
                return handleResponse(res, 404, 'error', `Job with id: ${officeId} not found`, null, 0);
            }
    ...
    async getAllOffices(req, res) {
        try {
            ...
            const { recommended, title, recent, location, page = 1, limit = 10, sort } = req.query;
            let queryObject = {};
            ...
            if (recent) {
                recent === true ? queryObject.createdAt = { $gte: new Date(new Date().setDate(new Date().getDate() - 30)) } : queryObject.createdAt = { $gte: new Date(new Date().setDate(new Date().getDate() - 30)) }
            }
    ...
    async getServicePosts(req, res) {
        try {
            ...
            const totalOffices = await Offices.countDocuments({ createdBy: req.user.id });
    ...
    async getSingleOffice(req, res) {
        try {
            ...
            if (!singleOffice) {
                return handleResponse(res, 404, 'error', `Job not found with ID: ${officeId}`, null, 0);
            }

            return handleResponse(res, 200, 'success', 'Job retrieved successfully', singleOffice, 1);
        } catch (error) {
            return handleResponse(res, 500, 'error', 'Something went wrong: ' + error.message, null, 0);
        }
    }
    async updateOffice(req, res) {
        try {
            ...
            const { params: { id: officeId } } = req;
            let office = await Offices.findById(officeId);
            if (!office || office.createdBy.toString() !== req.user.id) {
                return handleResponse(res, 404, 'error', `Office not found with ID: ${officeId}`, null, 0);
            }
            let deleteImages = []
            let newImages = []
            let keepImages = []
            ...
            if (keepImages.length > 0 && newImages.length > 0) {
                ...
                deleteImages = office.images.filter(image => !collectedImages.includes(image));
            ...
            return handleResponse(res, 200, 'success', 'Office updated successfully', office, 1);
        } catch (error) {
            return handleResponse(res, 500, 'error', 'Something went wrong: ' + error.message, null, 0);
        }
    }
    ...
    async getFavoriteOffices(req, res) {
        try {
            ...
            if (!favoriteOffices || favoriteOffices.length === 0) {
                return handleResponse(res, 200, 'success', 'No favorite offices found', null, 0);
            }
            // Successful response returning the favorite offices
            return handleResponse(res, 200, 'success', 'Favorite offices retrieved successfully', favoriteOffices, favoriteOffices.length);
        } catch (error) {
            return handleResponse(res, 500, 'error', 'Something went wrong: ' + error.message, null, 0);
        }
    }
