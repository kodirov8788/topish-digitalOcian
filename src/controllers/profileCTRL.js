...
class ProfileCTRL {
    async UpdateProfileVisibility(req, res) {
        try {
            ...
            return handleResponse(res, 200, 'success', 'Profile visibility updated successfully', null, 0);
        } catch (error) {
    ...
    async SendProfileAccessRequest(req, res) {
        try {
            ...
            if (!targetUserId) {
                return handleResponse(res, 400, 'error', 'Target user ID must be provided', null, 0);
            }

            if (!requesterId) {
                return handleResponse(res, 400, 'error', 'Requester user ID must be provided', null, 0);
            }

            if (requesterId === targetUserId) {
                return handleResponse(res, 400, 'error', 'Requester and target user cannot be the same', null, 0);
            }
            ...
            if (existingRequest) {
                ...
                return handleResponse(res, 200, 'success', 'Existing profile access request status updated to pending', null, 0);
            }
            ...
            await request.save();
            return handleResponse(res, 200, 'success', 'Profile access request sent successfully', null, 0);
        } catch (error) {
            return handleResponse(res, 500, 'error', 'Something went wrong: ' + error.message, null, 0);
        }
    }
    ...
    async GetProfileAccessRequests(req, res) {
        try {
            ...
            if (requests.length === 0) {
                return handleResponse(res, 200, 'success', 'No requests received', null, 0);
            }
        ...
        try {
            ...
            if (request.status === "pending") {
                ...
                if (!requestUser || !targetUser) {
                    return handleResponse(res, 404, 'error', 'One or both users not found', null, 0);
                }
                ...
                return handleResponse(res, 200, 'success', 'Request accepted and users are now friends', null, 0);
            } else {
                return handleResponse(res, 400, 'error', 'Request has already been processed', null, 0);
            }
        } catch (error) {
            return handleResponse(res, 500, 'error', 'Something went wrong: ' + error.message, null, 0);
        }
    ...
    async DeclineProfileAccessRequest(req, res) {
        try {
            ...
            if (request.status === "pending") {
                ...
                return handleResponse(res, 200, 'success', 'Request declined and users are no longer friends', null, 0);
            } else {
                return handleResponse(res, 400, 'error', 'Request has already been processed', null, 0);
            }
        } catch (error) {
    ...
    async ReverseAcceptanceOfProfileAccessRequest(req, res) {
        try {
            ...
            if (request.status === "accepted") {
                ...
                return handleResponse(res, 200, 'success', 'Friendship reversed and request status set to declined', null, 0);
            } else {
                return handleResponse(res, 400, 'error', 'Request is not in an accepted state', null, 0);
            }
        } catch (error) {
            return handleResponse(res, 500, 'error', 'Something went wrong: ' + error.message, null, 0);
        }
