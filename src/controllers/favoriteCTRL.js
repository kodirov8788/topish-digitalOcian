...
class FavoriteCTRL {
    async AddToFavorite(req, res, next) {
        try {
            ...
            if (currentUser.favorites.includes(favoriteUser.id)) {
                return handleResponse(res, 400, 'error', 'User already in favorites', null, 0);
            }

            currentUser.favorites.push(favoriteUser.id);
            await currentUser.save();

            return handleResponse(res, 200, 'success', 'User added to favorites', null, 1);
        } catch (error) {
    ...
    async GetFavoriteUser(req, res, next) {
        try {
            ...
            const favoriteId = req.params.favoriteId;
            if (!favoriteId) {
                return handleResponse(res, 400, 'error', 'No favorite user specified', null, 0);
            }
            ...
            if (!currentUser || !currentUser.favorites || currentUser.favorites.length === 0) {
                return handleResponse(res, 404, 'error', 'Favorite user not found in your favorites', null, 0);
            }
    ...
    async RemoveFromFavorite(req, res, next) {
        try {
            ...
            if (req.user.role === "JobSeeker") {
                return handleResponse(res, 400, 'error', "JobSeeker can't remove from favorites", null, 0);
            }
            const currentUser = await Users.findById(req.user.id);
            const favoriteId = req.params.favoriteId;
            // Check if the user is in favorites using ObjectId comparison
            if (!currentUser.favorites.some(favId => favId.toString() === favoriteId)) {
                return handleResponse(res, 404, 'error', "User not found in favorites", null, 0);
            }
            // Use MongoDB's $pull operator to remove the user from favorites efficiently
            await Users.findByIdAndUpdate(req.user.id, {
                $pull: { favorites: favoriteId }
            });
            return handleResponse(res, 200, 'success', "User removed from favorites", null, 0);
        } catch (error) {
            return handleResponse(res, 500, 'error', 'Something went wrong: ' + error.message, null, 0);
        }
    }
    async GetAllFavoriteUsers(req, res, next) {
        try {
            ...
            if (!req.user || !req.user.id) {
                return handleResponse(res, 401, 'error', "Unauthorized! Missing user ID.", null, 0);
            }
            ...
            if (!user || user.favorites.length === 0) {
                return handleResponse(res, 200, 'success', "Your favorites list is empty.", [], 0);
            }
            ...
            }).populate({ path: "resumeId" }).select("-password");
            ...
            return handleResponse(res, 200, 'success', "Favorite users retrieved successfully.", favorites, favorites.length, pagination);
        } catch (error) {
            return handleResponse(res, 500, 'error', 'Something went wrong: ' + error.message, null, 0);
        }
    }
