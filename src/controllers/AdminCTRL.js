...
class AdminCTRL {
    async getUsersForAdmin(req, res) {
        ...
        try {
            ...
            return handleResponse(res, 200, 'success', 'Users retrieved successfully', users, users.length, pagination);

        } catch (error) {
            // Log and return the error
            console.error('Error fetching users:', error);
            return handleResponse(res, 500, 'error', 'An error occurred while fetching the users.', null, 0);
        }
    }
    ...
    async getEmployersForAdmin(req, res) {
        try {
            ...
            const searchedUsers = await Users.find(query).populate("resumeId")
                .skip(skip)
                .limit(limit);
            ...
            return handleResponse(res, 200, 'success', 'Employers retrieved successfully', searchedUsers, searchedUsers.length, pagination);
        } catch (error) {
            console.error("Error in getAllEmployers function:", error);
            return handleResponse(res, error.status || 500, 'error', error.message || 'Something went wrong', null, 0);
        }
    }
    async getJobsForAdmin(req, res) {
        try {
            ...
            const {
                ...
                jobTitle, sort, recentjob, page = 1, limit = 10, numericFilters, jobType,
            } = req.query;
    ...
    async getOfficesForAdmin(req, res) {
        try {
            ...
            const { recommended, title, location, page = 1, limit = 10, sort } = req.query;
            let queryObject = {};
            ...
            return handleResponse(res, 200, 'success', 'Offices retrieved successfully', searchedOffice, searchedOffice.length, pagination);
        } catch (error) {
    ...
    async blockUserByAdmin(req, res) {
        try {
            ...
            if (user.blocked === undefined) {
                ...
                await user.save();
                return handleResponse(res, 200, 'success', 'User blocked successfully', user);
            }
            ...
            if (!user.blocked) {
                user.blocked = true;
                await user.save();
                return handleResponse(res, 200, 'success', 'User already had blocked field. Now set to blocked.', user);
            }

            return handleResponse(res, 200, 'success', 'User was already blocked', user);
        } catch (error) {
    ...
    async unblockUserByAdmin(req, res) {
        try {
            ...
            if (user.blocked === undefined) {
                ...
                await user.save();
                return handleResponse(res, 200, 'success', 'User unblocked successfully', user);
            }
            ...
            if (user.blocked) {
                user.blocked = false;
                await user.save();
                return handleResponse(res, 200, 'success', 'User already had blocked field. Now set to unblocked.', user);
            }

            return handleResponse(res, 200, 'success', 'User was already unblocked', user);
        } catch (error) {
    ...
    async sendNewsToAllUsers(req, res) {
        try {
            ...
            if (!title || !message) {
                return handleResponse(res, 400, 'error', 'Title and message are required');
            }
            ...
            return handleResponse(res, 200, 'success', 'News sent successfully', notifications);
