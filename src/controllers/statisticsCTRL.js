...
async function aggregateApplicantsCount(matchStage) {
    ...
    const total = (jobsCount[0] ? jobsCount[0].count : 0) + (quickJobsCount[0] ? quickJobsCount[0].count : 0);
    return total;
}
class StatisticsCTRL {
    async getJobSeekerCount(req, res) {
        try {
            ...
            const rate = thisPeriodCount > previousPeriodCount ? "up" : thisPeriodCount < previousPeriodCount ? "down" : "steady";

            // Calculate the percentage change, avoiding division by zero
            let thisPeriodPercentage = previousPeriodCount > 0 ? ((thisPeriodCount - previousPeriodCount) / previousPeriodCount) * 100 : 0;
            ...
            return handleResponse(res, 200, 'success', 'Job seekers count information retrieved successfully', {
    ...
    async getEmployerCount(req, res) {
        try {
            ...
            const rateStatus = thisMonthCount > previousPeriodCount ? "up" : thisMonthCount < previousPeriodCount ? "down" : "steady";
            let thisPeriodPercentage = previousPeriodCount > 0 ? ((thisMonthCount - previousPeriodCount) / previousPeriodCount) * 100 : 0;
            thisPeriodPercentage = thisPeriodPercentage.toFixed(2); // Two decimal precision

            // Return the counts, rate, and percentage in the response
            return handleResponse(res, 200, 'success', 'Employer count information retrieved successfully', {
    ...
    async getJobsCount(req, res) {
        try {
            ...
            const selectedDayJobsCount = await Jobs.countDocuments({ createdAt: { $gte: selectedDay, $lte: selectedDayEnd } });
            const selectedDayQuickJobsCount = await QuickJobs.countDocuments({ createdAt: { $gte: selectedDay, $lte: selectedDayEnd } });
            ...
            const thisMonthJobsCount = await Jobs.countDocuments({ createdAt: { $gte: thisMonthStartDate, $lte: today } });
            const thisMonthQuickJobsCount = await QuickJobs.countDocuments({ createdAt: { $gte: thisMonthStartDate, $lte: today } });
            const thisMonthCount = thisMonthJobsCount + thisMonthQuickJobsCount;

            // Calculate the total jobs count (Jobs + QuickJobs)
            const totalJobsCount = await Jobs.countDocuments({}) + await QuickJobs.countDocuments({});
            ...
            return handleResponse(res, 200, 'success', 'Job counts information retrieved successfully', {
                totalJobsCount,
                thisMonthCount,
                rateStatus,
                thisPeriodPercentage: `${thisPeriodPercentage}%`,
    ...
    async getApplicantsCount(req, res) {
        try {
            ...
            const selectedDayCount = await aggregateApplicantsCount({ createdAt: { $gte: queryDate, $lte: endDate } });
            ...
            const thisMonthCount = await aggregateApplicantsCount({ createdAt: { $gte: thisMonthStartDate, $lte: todayEndDate } });
            ...
            return handleResponse(res, 200, 'success', 'Applicants count information retrieved successfully', {
                totalJobsCount,
                thisMonthCount,
                rateStatus,
                thisPeriodPercentage: `${thisPeriodPercentage}%`,
