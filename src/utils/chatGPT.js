...
async function handleChatGPT(socket, data) {
    ...
    try {
        ...
        if (results.length > 0) {
            ...
            const completedData = aiCompletion.choices[0].message.content.trim().replace(/\*/g, '');
        ...
        return socket.emit('chatGPTAnswer', 'Kechirasiz, qidiruvda xatolik yuz berdi. Iltimos, qayta urinib ko\'ring.');
    }
}
